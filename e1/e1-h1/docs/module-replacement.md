...
the module-only inputs, the generated execution recipe records the exact
Verilator command and run executable owned by the C++ generator, and
`e1/tools/run_module_dpi_verilator.py` emits the execution report showing each
probe was built and run from that recipe. This keeps the CPU,
