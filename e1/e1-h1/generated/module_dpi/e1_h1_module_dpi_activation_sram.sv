...
module e1_h1_module_dpi_activation_sram;
  ...
  task automatic check_initialized(input int cycle);
    if (e1_h1_module_dpi_compare_u32("initialized_q", cycle, int'({31'd0, u_imp1.initialized_q}), int'({31'd0, u_imp2.initialized_q})) == 0) begin
      $fatal(1, "activation_sram mismatch initialized_q cycle %0d", cycle);
    end
  endtask
  ...
  initial begin
    e1_h1_module_dpi_begin("activation_sram", "module_only_config_sram");
    e1_h1_module_dpi_case("activation_sram", "reset_empty");
    e1_h1_module_dpi_case("activation_sram", "single_config_read");
    e1_h1_module_dpi_case("activation_sram", "parameterized_size");
    clk_i = 1'b0;
    rst_ni = 1'b0;
    tick();
    check_initialized(-1);
    ...
    for (int cycle = 0; cycle < 3; cycle++) begin
      ...
      e1_h1_module_dpi_cycle("activation_sram", cycle, phase_name(cycle));
      if (e1_h1_module_dpi_phase_signal("activation_sram", "probe_cycle_phase_o", cycle, cycle, int'(probe_cycle_phase_o)) == 0) begin
        $fatal(1, "activation_sram phase signal mismatch cycle %0d", cycle);
      end
      tick();
